...
st.error(f"Error connecting to Journal Vault: {e}"); return
if df.empty:
st.info("Your private vault is currently empty."); return
...
with c3:
    st.metric("LIVE", live_count); st.metric("DEMO", demo_count); st.metric("HINDSIGHT", hind_count)

st.divider()
cal_filter = st.radio("TF", ["All Trades", "By Year", "By Month", "By Week", "By Day"], horizontal=True, label_visibility="collapsed")
f1, f2, f3 = st.columns(3)
with f1: sel_model = st.selectbox("MODEL", ["ALL MODELS"] + sorted(df['model_name'].unique().tolist()))
with f2: sel_var = st.selectbox("VARIATION", ["ALL VARIATIONS"] + sorted(df['model_var'].dropna().unique().tolist()))
with f3: sel_res = st.selectbox("RESULT", ["ALL RESULTS", "WIN", "LOSS", "BE"])

df['date_dt'] = pd.to_datetime(df['date'])
today = datetime.now().date()
if cal_filter == "By Week": df = df[df['date_dt'].dt.date >= (today - timedelta(days=7))]
elif cal_filter == "By Day": df = df[df['date_dt'].dt.date == today]
...
header = f"{row['model_name']} | {row['market']} | {row['date_dt'].strftime('%d %b %Y')} | {round(row['rr'], 2)}R"
with st.expander(header):
if st.session_state.get('editing_id') == row['id']:
with st.form(f"edit{row['id']}"):
st.markdown("### 🛠️ EDIT MASTER DATA")
...
e_tf = st.text_input("TF", value=row.get('entry_tf', '')).upper()
with ec2:
e_type = st.text_input("ENTRY TYPE", value=row.get('entry_type', '')).upper()
e_sess = st.text_input("SESS", value=row.get('session', '')).upper()
e_time = st.text_input("TIME", value=row.get('entry_time', ''))
n_l = ["NONE", "LOW", "MEDIUM", "HIGH", "NFP/CPI"]
e_news = st.selectbox("NEWS", n_l, index=n_l.index(row.get('news_impact', 'NONE')))
with ec3:
...
e_notes = st.text_area("NOTES", value=row['notes'], height=100)
if st.form_submit_button("💾 UPDATE"):
new_rr = e_tp / e_sl if e_sl != 0 else 0
up = {"model_name": e_mod, "model_var": e_var, "market": e_mkt, "entry_tf": e_tf, "entry_type": e_type, "session": e_sess, "entry_time": e_time, "news_impact": e_news, "result": e_res, "risk_pc": e_risk, "duration_mins": e_dur, "notes": e_notes, "tp_handles": e_tp, "sl_handles": e_sl, "rr": new_rr}
...
c1, c2, c3 = st.columns(3)
with c1:
st.write(f"TIME: {row.get('entry_time', 'N/A')}"); st.write(f"SESS: {row.get('session', 'N/A')}")
st.write(f"ENTRY: {row.get('entry_type', 'N/A')}"); st.write(f"NEWS: {row.get('news_impact', 'NONE')}")
with c2:
st.write(f"TF: {row.get('entry_tf', 'N/A')}"); st.write(f"VAR: {row.get('model_var', 'N/A')}")
st.write(f"DUR: {row.get('duration_mins', 0)}m")
with c3:
st.write(f"RISK: {row.get('risk_pc', 0)}% "); st.write(f"RESULT: :{color}[{res}]")
st.write(f"MODE: {'HINDSIGHT' if row.get('hindsight') else row.get('type', 'LIVE')}")
...
if st.button("✏️ EDIT FULL DATA", key=f"ed_{row['id']}"):
st.session_state.editing_id = row['id']; st.rerun()
st.info(f"NOTES: {row.get('notes', 'N/A')}"); st.divider()