...
st.set_page_config(page_title="ICT_MASTER_TERMINAL_V5.8", layout="wide")
...
with tabs[0]:
    ...
    m_logic = st.text_area("CORE_LOGIC_&_MACRO_RULES", height=200)
    ...
    if not models:
        ...
        with st.form("log_form", clear_on_submit=True):
            ...
            with c1:
                ...
                ent_type = st.text_input("DETAILS (e.g. FVG)")
            with c2:
                ent_time = st.text_input("TIME (HH:MM)")
                ent_tf = st.selectbox("TIMEFRAME", ["1m", "5m", "15m", "1h", "4h", "Daily"])
                ent_sess = st.text_input("SESSION").upper()
                dur = st.number_input("DURATION (MINS)", min_value=1, value=15)
            with c3:
                news = st.selectbox("NEWS", ["NONE", "LOW", "MEDIUM", "HIGH (RED)"])
            ...
            if st.form_submit_button("SAVE_ENTRY"):
                if not mkt or not ent_time:
                    st.error("MISSING DATA: Market and Time are required.")
                else:
...
def render_segmented_analytics(df_all, suffix):
    ...
    color_map = {"GOLD_MACRO": "#FFD700", "02:00 Hour": "#1f77b4", "03:00 Hour": "#ff7f0e", "04:00 Hour": "#2ca02c", "05:00 Hour": "#d62728", "06:00 Hour": "#9467bd"}
    ...
    for result_type, title, emoji in [("WIN", "THE WINNERS' CIRCLE", "🏆"), ("BE", "THE BREAK-EVEN DEFENSE", "🛡️"), ("LOSS", "THE AUTOPSY", "💀")]:
        ...
        if sub_df.empty:
            st.write(f"No {result_type} trades recorded.")
        else:
            sub_df['Cat'] = sub_df['entry_time'].apply(get_color_cat)
            c1, c2 = st.columns([2, 1])
            with c1:
                fig = px.bar(sub_df.sort_values('entry_time'), x='entry_time', color='Cat', title=f"{result_type} Pulse (1m)", color_discrete_map=color_map)
                fig.update_layout(bargap=0.4); st.plotly_chart(fig, use_container_width=True, key=f"bar_{result_type}_{suffix}")
            with c2:
                st.metric(f"AVG {result_type} DURATION", f"{round(sub_df['duration_mins'].mean(), 1)}m")
                st.plotly_chart(px.pie(sub_df, names='entry_tf', title="Timeframes", hole=0.4), key=f"p_tf_{result_type}_{suffix}")
            
            p1, p2, p3, p4, p5 = st.columns(5)
            with p1: st.plotly_chart(px.pie(sub_df, names='market', title="Markets", hole=0.4), key=f"p_mkt_{result_type}_{suffix}")
            with p2: st.plotly_chart(px.pie(sub_df, names='session', title="Sessions", hole=0.4), key=f"p_ses_{result_type}_{suffix}")
            with p3: st.plotly_chart(px.pie(sub_df, names='target', title="Targets", hole=0.4), key=f"p_tar_{result_type}_{suffix}")
            with p4: st.plotly_chart(px.pie(sub_df, names='news_impact', title="News", hole=0.4), key=f"p_nws_{result_type}_{suffix}")
            with p5: st.plotly_chart(px.pie(sub_df, names='entry_info', title="Setups", hole=0.4), key=f"p_ent_{result_type}_{suffix}")
        st.divider()
...
with tabs[4]:
    ...
    for _, row in hist_df[::-1].iterrows():
        with st.expander(f"{row['date']} | {row['entry_time']} | {row['market']} | {row['result']}"):
            col_a, col_b = st.columns([2, 1])
            with col_a:
                st.write(f"**TF:** {row['entry_tf']} | **Session:** {row['session']} | **Target:** {row['target']}")
                st.write(f"**Duration:** {row['duration_mins']}m | **News:** {row['news_impact']}")
