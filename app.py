...
with tabs[0]:
    ...
    m_name = st.text_input("MODEL NAME").upper()
    m_sess = st.multiselect("SESSIONS", ["ASIA", "LONDON", "NY AM", "NY PM"])
    m_logic = st.text_area("CORE LOGIC", height=150)
...
with tabs[1]:
    ...
    if not models: st.warning("Create a Model in Architect first.")
        ...
        with st.form("forge_form", clear_on_submit=True):
            ...
            with c2:
                tm = st.text_input("TIME")
                tf = st.text_input("TF").upper()
                sess = st.text_input("SESSION").upper()
                dur = st.number_input("DURATION", 1, 500, 15)
            with c3:
                nws = st.text_input("NEWS").upper()
                res = st.text_input("RESULT").upper()
...
def render_analytics(df, suffix):
    ...
    with c_bar:
        st.plotly_chart(px.bar(df.sort_values('entry_time'), x='entry_time', y='rr', color='result', color_discrete_map=colors).update_layout(bargap=0.3), use_container_width=True, key=f"main_bar_{suffix}")
    with c_pie:
        st.plotly_chart(px.pie(df, names='result', hole=0.5, color='result', color_discrete_map=colors), use_container_width=True, key=f"main_pie_{suffix}")
...
with tabs[4]:
    ...
    if all_t.empty:
        ...
        cal_filter = st.radio("FILTER", ["All Sales", "By Year", "By Month", "By Week", "By Day"], horizontal=True, label_visibility="collapsed")
...
with tabs[5]:
    ...
    p, r, y = st.number_input("START", 5000), st.number_input("MONTHLY %", 5.0), st.number_input("YEARS", 5)
    bal, data = p, []
